...
export const WeatherLocationModal: React.FC<WeatherLocationModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    recipeAPI.getWeatherLocation()
      .then(loc => setSaved(loc))
      .catch(() => {})
  ...
  const selectLocation = async (loc: Location) => {
    ...
    try {
      const data = await recipeAPI.saveWeatherLocation(loc.latitude, loc.longitude, label);
      setSaved(data);
      setJustSaved(true);
  ...
  const clearLocation = async () => {
    try {
      await recipeAPI.deleteWeatherLocation();
      setSaved(null);
    } catch { /* ignore */ }
  };
    ...
    >
      ...
      >
        <VStack gap={5} align="stretch">
          ...
          {initialLoading ? (
            <HStack justify="center" py={4}><Loader2 className="spin" size={20} /></HStack>
          ) : (
            <>
              {saved && (
