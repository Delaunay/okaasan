"""Audio streaming with pluggable backends (direct serving, transcode via ffmpeg)."""
...
log = logging.getLogger("okaasan.music.streamer")
...
class AudioStreamer(ABC):
    """Abstract base for audio streaming strategies."""
...
class DirectAudioStreamer(AudioStreamer):
    """Serve audio files directly with range request support."""
...
class TranscodeAudioStreamer(AudioStreamer):
    """Transcode audio via ffmpeg to MP3 for universal browser support."""
...
class _BackgroundKill:
    """Starlette background task that kills ffmpeg when the response finishes."""
...
_DIRECT_PLAYABLE = {"mp3", "m4a", "aac", "ogg", "opus"}


def get_audio_streamer(file_path: str) -> AudioStreamer:
    """Select the appropriate streamer based on file format."""
    ext = file_path.rsplit(".", 1)[-1].lower() if "." in file_path else ""
    if ext in _DIRECT_PLAYABLE:
        return DirectAudioStreamer()
    return TranscodeAudioStreamer()
